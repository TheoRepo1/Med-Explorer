...
def load_all_data():
    """
    Charge le dataframe des médicaments et la matrice des embeddings.
    Utilise st.cache_data pour une performance optimale.
    """
...
if data is None or embeddings is None:
    st.error("Fichiers de données ou d'embeddings non trouvés. Veuillez (re)construire l'application avec `docker-compose up --build`.")
else:
